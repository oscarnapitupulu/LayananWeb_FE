...
--
-- Host: 127.0.0.1
-- Generation Time: 12 Des 2022 pada 06.24
-- Versi Server: 5.6.20
-- PHP Version: 5.5.15
...
--
DELIMITER //
CREATE TRIGGER `HapusAdministrasi` AFTER DELETE ON `administrasi`
 FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER //
CREATE TRIGGER `TambahAdministrasi` AFTER INSERT ON `administrasi`
 FOR EACH ROW BEGIN
...
CREATE TABLE IF NOT EXISTS `identitas_siswa` (
`Id_Identitas_Siswa` int(11) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `orang_tua_wali` (
...
--
DELIMITER //
CREATE TRIGGER `HapusOrangTuaWali` AFTER DELETE ON `orang_tua_wali`
 FOR EACH ROW BEGIN
...
DELIMITER ;
DELIMITER //
CREATE TRIGGER `TambahOrangTuaWali` AFTER INSERT ON `orang_tua_wali`
 FOR EACH ROW BEGIN
...
(1, 'Andi Santosoku', 'pegawai1', 'pegawai1', 'pegawai', 'aktif', '2020-09-16 19:09:20', '2022-12-12 05:09:44'),
(6, 'Tiara Ekylia Eriza Putri', 'admin', 'admin', 'admin', 'aktif', '2020-09-16 18:32:57', '2022-03-06 22:56:09');
...
ALTER TABLE `administrasi`
 ADD PRIMARY KEY (`id_administrasi`), ADD KEY `id_identitas_siswa` (`id_identitas_siswa`);

--
-- Indexes for table `identitas_siswa`
--
ALTER TABLE `identitas_siswa`
 ADD PRIMARY KEY (`Id_Identitas_Siswa`), ADD UNIQUE KEY `NISN` (`NISN`), ADD UNIQUE KEY `NIK` (`NIK`);

--
-- Indexes for table `orang_tua_wali`
--
ALTER TABLE `orang_tua_wali`
 ADD PRIMARY KEY (`Id_Orang_Tua_Wali`), ADD KEY `Id_Identitas_Siswa` (`Id_Identitas_Siswa`);
